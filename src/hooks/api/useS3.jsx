...
export function useS3({ defaultPrefix }) {
  ...
  async function listBucketObjects(objName) {
    try {
      ...
      return objects.map(({ ETag, Size, key, shortObjectKey }) => ({ id: ETag + key, objectKey:key, name: shortObjectKey, ...formatFileSize(Size) }))
    } catch (error) {
  ...
  async function uploadObjectToBucket({ file }) {
    ...
    try {
      ...
      return { id: response.ETag + file.name, key: response.key, name: file.name, ...formatFileSize(file.size) }
    } catch (error) {
      setError(error.message)
      throw error
    } finally {
